...
def test_get_unit_supplementary_data_200_is_logged(caplog, client, database):
    ...
    response = client.get(
        f"/v1/unit_data?dataset_id={test_dataset_id}&unit_id={test_unit_id}"
    )
    ...
    assert (
        caplog.records[1].message == "Unit supplementary data outputted successfully."
    )
...
def test_get_unit_supplementary_data_debug_logs(caplog, client, database):
    """
    ...
    assert caplog.records[4].message == "Unit supplementary data: {'test': 'data'}"
