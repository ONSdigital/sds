...
def publish_schema_to_sds(schema, survey_id):
    ...
    Parameters:
        schema: A dict of the schema bein published that matches the agreed schema structure
    ...
    base_url = "https://34.160.249.218.nip.io"

    # Make request to IAP of SDS load balancer
    response = _make_iap_request(
        f"{base_url}/v1/schema?survey_id={survey_id}", audience, key_file, schema
    )
...
def _get_client_id(project_id, key_file) -> str:
    ...
    try:
        ...
        # Resume to use original SA stored in GOOGLE_APPLICATION_CREDENTIALS
        cmd_resume_auth = (
